...
public class NumberTypeRule extends AbstractTypeRule {
    ...
    public NumberTypeRule(Double lowerBound, Double upperBound) {
    ...
    public boolean matches(Node node) {
        if (node instanceof StringNode && castStringsAsNumbers) {
            ...
            try {
                ...
                return contains(doubleValue);
            } catch (NumberFormatException e) {
                return false;
            }
        }
        if (node instanceof FloatingNode) {
            return contains(((FloatingNode) node).getValue().doubleValue());
        }
        if (node instanceof IntegerNode) {
            return contains(((IntegerNode) node).getValue().doubleValue());
        }
        return false;
    }
    ...
    private boolean contains(Double d) {
        return lowerBound == null || upperBound == null || lowerBound <= d && d <= upperBound;
    }
}
